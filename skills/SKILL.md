...
|----------|-------------|
| `embeddings/embed` | Generate embeddings for one or more input strings using the configured embeddings provider. Returns an OpenAI-compatible response: `{ object: "list", data: [{ object, index, embedding }], model, usage }`.. Params: `input` (string,array, required), `model` (string) |
